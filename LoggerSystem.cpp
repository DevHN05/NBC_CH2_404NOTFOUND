...
void LoggerSystem::ChoiceGarbageCollector(int DexBonus)
{
    ...
    PrintMainArea({
        ...
        "이 폭주한 가비지 컬렉션이 모든 버그의 원흉일까요? 누군가가 의도적으로 건든 흔적이 있습니다",
        "우선, 조심스럽게 접근하죠. 가비지 컬렉션에게 잘못 걸리면 당신마저 쓰레기 데이터로 분류되어 삭제될수도 있거든요."
    });
    Gm.ClearLogs();
    Gm.AddLog("1. 가비지 컬렉션의 시선을 피해 조심스럽게 기습합니다.");
    Gm.AddLog("   (판정값 15, 민첩 보정 +" + to_string(DexBonus) + ")");
    Gm.AddLog("2. 가비지 컬렉션도 코드덩어리. 정면으로 싸웁니다.");
    Gm.AddLog("   (전투 발생)");
}
...
{
    PrintMainArea({
        "조심스럽게 가비지 컬렉션의 눈을 피해 기습을 노립니다!",
        "경계가 삼엄해 선공을 하진 못했지만, 깨진 물약 액터를 발견해 hp를 회복합니다."
    });
}
